...
### Aizarani cell types
The datasets, mapped to the Aizarani dataset:
...
cellTypes <- unique(aizarani$paper.cluster.names)
...
library(scales)
clusterNames <- unique(aizarani$paper.clusters)
...
In Figure 1, NK, NKT, T cells are mainly found (paper) clusters 1, 5, 3, 28, 12.  In the heatmap, the main marker genes for this cluster are: KLRB1, CD8A, PTPRC, CD3E
...
"genes.1" = c("CCL5", "NKG7", "TRCGC1", "KLRD1", "RUNX3", "GNLY", "GZMK", "GZMH", "CST7", "TRBC2"),
"genes.3" =  c("IL7R", "KLRB1", "LTB", "CD69", "SPOCK2", "TNFAIP3", "CXCR4", "TRAC", "TRBC2", "TXNIP"),
"genes.5" = c("CCL4", "CCL4L2", "IL2RB", "NKG7", "CCL3L3", "CMC1", "XCL1", "CD7", "KLRF1"),
"genes.12" = c("HSPA1A", "HSPA1B", "DNAJB1", "HSPA6", "HSP90AA1", "HSPH1", "IL2RB", "NKG7", "CD7", "IFNG"),
"genes.28" = c("SRGN", "TNFAIP3", "ZFP36L2", "AIM1", "CREM", "CXCR4", "RGS1", "IL7R", "XCL1")
)



genes.kup.paper <- unique(as.character(unlist(genes.kup)))


p1 <- DoHeatmap(reference, features=genes.kup.paper) + NoLegend() + ggtitle("seruat clusters")

p2 <- VlnPlot(reference, features=genes.kup[['genes.heatmap']])
...
In Figure 1 of the paper, Kupfer cells are mainly found in clusters 6, 23, 25, and 31.  The main genes marking this cluster is CD163, MARB and VSIG4.


```{r}
VlnPlot(reference, features=c("CD163", "MARB", "VSIG4"))
```
...
VlnPlot(reference, features=c("ALB", "TF", "CYP3A4", "CYP2E1", "APOB", "ASGR1", "PCK1", "HP", "ASS1", "APOE"))
...
It's not clear what cluster 16 is - 

#### Zonation

Checking some possible markers of zonation:

```{r fig.width=10, fig.height=10}
genes.zonation <- c("GLUL", "CYP2E1", "ASS1", "ALB", "CYP2F2", "CYP1A2", "ARG1", "PCK1", "ASL", "GSTM3", "SDHD")
FeaturePlot(reference, reduction="umap", features = genes.zonation)
```
...
GLUL: more central - 3 central, 7, 9, 5 peri-portal
CYP2E1: lower central and peri-portal, more mid - 5 mid
ALB: lower central, higher portal: 7 central, 5, 3, 9 portal
CYPA1: lower central, higher portal: 9 central, 5, 7 portal
PCK1: lower central, higher portal: central 3, 5, 7, 9 portal

```{r fig.height=10, fig.width=10}
VlnPlot(subset(reference, seurat_clusters %in% c( 3, 5, 7, 9, 16)), features=genes.zonation)
```
