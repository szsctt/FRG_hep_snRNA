...
```{r}
markers <- c("ASS1", "CYP2E1", "ARG1", "ASL1", "GLUL", "CY1A2", "LRG5", "AXIN2", "KIAA0319L")
markers
```
...
for (marker in markers[markers %in% rownames(hep)]) {
  plotlist[[marker]] <- FeaturePlot(hep, marker, split.by="dataset.type", order=TRUE, keep.scale="feature") + theme(legend.position = "right")
}
