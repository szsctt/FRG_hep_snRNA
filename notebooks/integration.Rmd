...
cells[['LK03C_REDHV']] <- ifelse(cells[['sample_type']] == "LK03C_REDHV",
                                 "LK03C_REDHV", NA)

cells[['LK03V_REDHC']] <- ifelse(cells[['sample_type']] == "LK03V_REDHC",
                                 "LK03V_REDHC", NA)
```
...
gene_of_interest <- function(gene) {
  ...
  if (detected) {
    ...
    print(p1)
    plotlist <- FeaturePlot(cells, features = gene, split.by="sample_type", 
...
```{r}
 FeaturePlot(subset(cells, sample_type== 'LK03C_REDHV' | sample_type== 'LK03V_REDHC'), 
             reduction='umap', features = aav_genes, 
            order=TRUE, split.by="sample_type")
```

For only the LK03C_REDHV samples:

```{r fig.width=7, fig.height=3}
 FeaturePlot(subset(cells, sample_type== 'LK03C_REDHV'), 
             reduction='umap', features = aav_genes, 
...
```{r  fig.width=7, fig.height=3}
 FeaturePlot(subset(cells, sample_type== 'LK03V_REDHC'), 
             reduction='umap', features = aav_genes, 
            order=TRUE, blend=TRUE, cols=c('lightgrey','red', 'blue'))
