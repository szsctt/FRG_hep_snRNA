...
Something strange is going on in cluster 6 - the top 5 genes are all mitochondrial genes.  These are probably low-quality cells?
...
cells.conservedMarkers <- list()


for (i in unique(cells.meta$seurat_clusters)) {
  cells.conservedMarkers[[i]] <- FindConservedMarkers(immune.combined, ident.1 = 0, grouping.var = "sample_type", verbose = FALSE)
...
We would next like to know which markers define the clusters, and are conserved across the conditions.
...
.  Seurat uses differential expression to find these, by comparing the genes expressed in one cluster against one or more other clusters.  Therefore, we need to specify which clusters we want to compare.  Here, I compare the genes for each cluster against every other cluster.
